...
var JotForm = {
    ...
    tempStripeCEForms: ['230024636370952', '230894369312966', '33361556326858', '230841251513446', '62366391795266', '220392206656353'],
    ...
    encryptFields: function () {
        return JotCrypto.encrypt(
            JotForm.encryptionPublicKey
        ).then(function (enc) {
            setEncryptedKeyToForm(enc.encryptedAESKey);
            ...
            return shouldSubmitFormAfterEncrypt(encFields);
        });
    },
    /**
            ...
            form.observe('submit', function (e) {
                ...
                if(JotForm.isEncrypted) {
                    ...
                    if (JotForm.encryptionProtocol === 'JF-CSE-V2') {
                        e.stop();
                        JotForm.encryptFields().then(function(submitFormAfterEncrypt) {
  ...
  /**
    ...
    updateMatrixInputs: function(id, target) {
        ...
        hiddenInputs.each(function(i) {
          if (i.id.indexOf('mobile') < 0) {
            i.id += '_mobile';
            i.setAttribute('data-name', i.getAttribute('name'));
            i.removeAttribute('name');
          }
        });
        var hiddenLabels = $(hiddenMatrix).select('.jfField-sublabel');
        hiddenLabels.each(function(i) {
            if (i.getAttribute('for').indexOf('mobile') < 0) {
                i.setAttribute('for', i.getAttribute('for') + '_mobile');
            }
        });
        ...
        var visibleLabels = $(visibleMatrix).select('.jfField-sublabel');
        visibleLabels.each(function(i) {
            if (i.getAttribute('for').indexOf('mobile') > 0) {
                i.setAttribute('for', i.getAttribute('for').replace('_mobile', ''));
            }
        });
...
function setEncryptedValue(a) {
    ...
    var fieldLine = field.up('li.form-line');
    var fieldType = fieldLine ? fieldLine.readAttribute('data-type') : '';

    //handle duplication of textareas, by forcefully encrypting the hidden previous sibling
    if (fieldType == "control_textarea") {
        var allFields = $$('[name="'+field.name+'"]');
        for (x=0; x < allFields.length ; x++) {
            allFields[x].value = encryptedValue;
        }
        return;
    }
    ...
    if (sendAsHiddenField.indexOf(fieldType) !== -1 || field.tagName == "SELECT" || isMixedEmailType) {
        ...
        var form = $$('.jotform-form')[0];
        form.insert(new Element('input', {type: 'hidden', name: field.name}).putValue(encryptedValue));
        if (fieldType == 'control_matrix') {
            // remove name to avoid duplication
            field.name = "";
        }
...
function setEncryptedKeyToForm(key) {
    var form = document.querySelector('.jotform-form');
    if (form) {
        ...
        input.setAttribute('name', 'submissionEncryptionKey');
        input.value = key;
        form.appendChild(input);
    }
}

function shouldSubmitFormAfterEncrypt(fields) {
    // payment fields that will submit the form on their own
    var selfSubmitFields = [
        "control_stripe",
        "control_braintree",
    ...
    fields.forEach(function (f) {
        var fieldLine = f.field.up('li.form-line');
        if (!fieldLine) {
            return;
        }
        var fieldType = fieldLine.readAttribute('data-type');
        if (!hasSelfSubmitField) {
            hasSelfSubmitField = selfSubmitFields.indexOf(fieldType) > -1;
        }
    });
